component dislayName="bravoAdminApplicationFile" output=true hint=""{
    ...
    this.directories                            = [ 'adminpanel', 'login', 'auth' ];
    ...
    public boolean function onApplicationStart(){
        ...
        if( this[ 'siteOn' ] ){
            ...
            //invoke Jsoup
            application[ 'adminPanel' ][ 'java' ][ 'jSoup' ] = createObject( "java", "org.jsoup.Jsoup" );

            //Build the navigation struct
            application[ 'adminPanel' ][ 'metaData' ]   = {};
            application[ 'adminPanel' ][ 'metaData' ]   = variables.createNavObject( application.adminPanel.components.site.controllers );


            //Insert href path for pages
            variables.buildHrefForPages();

            //define a struct for edit user links for chrome, I'm just being lazy and putting this here instead of in the chrome controller. For now ast least.
            APPLICATION[ 'editUserLink' ]                   = {};
            APPLICATION[ 'editUserLink' ][ 'User' ]         = application.adminPanel.components.utility.strings.Strings_GeneratePageHref_TASK( 'userEditing' );
            APPLICATION[ 'editUserLink' ][ 'Special' ]      = application.adminPanel.components.utility.strings.Strings_GeneratePageHref_TASK( 'tradeEditing' );
            APPLICATION[ 'editUserLink' ][ 'Site' ]         = application.adminPanel.components.utility.strings.Strings_GeneratePageHref_TASK( 'siteEditing' );
            APPLICATION[ 'editUserLink' ][ 'Admin' ]        = application.adminPanel.components.utility.strings.Strings_GeneratePageHref_TASK( 'siteEditing' );
            ...
            fileWrite( expandPath( '/config/metaData/pages.json' ), serializeJSON( application.adminPanel.metaData.pages ) );
    ...
    public function onSessionEnd(){
        ...
        application.adminPanel.components.utility.sessionManager.SessionManager_DestoryUserSession_TASK();
    ...
    public function onRequestEnd(){
        ...
        if( application[ 'inPanel' ] ){
            ...
            if( 
                ...
                session.adminPanel.user.lastVisited = application.adminPanel.components.utility.strings.Strings_GeneratePageHref_TASK( 'dashboard' );
    ...
    public void function buildComponentSets(){
        ...
        APPLICATION[ 'adminPanel' ][ 'components' ]?: structInsert( APPLICATION[ 'adminPanel' ], 'components', structNew() );

        //load up components for pages and utilities, store them into the admin panel struct
        APPLICATION[ 'adminPanel' ][ 'components' ][ 'site' ]?:     structInsert( APPLICATION[ 'adminPanel' ][ 'components' ], 'site',      variables.createSiteMVCComponents() );
        APPLICATION[ 'adminPanel' ][ 'components' ][ 'utility' ]?:  structInsert( APPLICATION[ 'adminPanel' ][ 'components' ], 'utility',   variables.createSiteUtilityComponents() );
    ...
    private void function buildHrefForPages(){
        ...
        for( key in application[ 'adminPanel' ][ 'metaData' ][ 'pages' ] ){

            structInsert( application[ 'adminPanel' ][ 'metaData' ][ 'pages' ][ key ], 'href', application.adminPanel.components.utility.strings.Strings_GeneratePageHref_TASK( application[ 'adminPanel' ][ 'metaData' ][ 'pages' ][ key ].alias ) );


        }

        for( sections in application[ 'adminPanel' ][ 'metaData' ][ 'navigation' ] ){

            for( key in application[ 'adminPanel' ][ 'metaData' ][ 'navigation' ][ sections ] ){

                if( this.debug ){
                    writeDump( application[ 'adminPanel' ][ 'metaData' ][ 'navigation' ][ sections ][ key ] )
                }

                if( isStruct( application[ 'adminPanel' ][ 'metaData' ][ 'navigation' ][ sections ][ key ] ) ){

                    structInsert( application[ 'adminPanel' ][ 'metaData' ][ 'navigation' ][ sections ][ key ], 'href', application.adminPanel.components.utility.strings.Strings_GeneratePageHref_TASK( application[ 'adminPanel' ][ 'metaData' ][ 'navigation' ][ sections ][ key ].alias )   );
